  ...
  class LocalCurrencyRateCacheAlgebra[F[_]: Async] extends CurrencyRateCacheAlgebra[F] {
    ...
    override def getRates(key: String): F[Error Either Rate] =
      localCache.get(key) match {
        ...
        case None       => Async[F].pure(Left(Error.KeyNotFoundInCache(key, s"Key not found in cache")))
      }

    override def updateRates(key: String, rate: Rate, timeoutInSeconds: Long): F[String] = {
